      ...
      <ContentWrapper>
        <RowBetween>
          <TYPE.body>
            ...
            <Link lineHeight={'145.23%'} href={'https://netzexplorer.io/address/' + account} target="_blank">
              {' '}
              {account?.slice(0, 42)}{' '}
            </Link>
          </TYPE.body>
            ...
            <DropdownWrapper>
              ...
              {showDropdown && (
                <Flyout>
                  <AutoColumn gap="0px">
                    {positions?.map((p, i) => {
                      if (p.pair.token1.symbol === 'WNETZ') {
                        p.pair.token1.symbol = 'NETZ'
                      }
                      if (p.pair.token0.symbol === 'WNETZ') {
                        p.pair.token0.symbol = 'NETZ'
                      }
                      return (
