...
function TxnList({ transactions, symbol0Override, symbol1Override, color }) {
  ...
  const ListItem = ({ item }) => {
    if (item.token0Symbol === 'WNETZ') {
      item.token0Symbol = 'NETZ'
    }

    if (item.token1Symbol === 'WNETZ') {
      item.token1Symbol = 'NETZ'
    }
    ...
    return (
      <DashGrid style={{ height: '48px' }}>
        ...
        <DataText area="value">
          {currency === 'NETZ' ? 'Ξ ' + formattedNum(item.valueETH) : formattedNum(item.amountUSD, true)}
        </DataText>
        {!below780 && (
          <>
        ...
        {!below1080 && (
          <DataText area="account">
            <Link color={color} external href={'https://netzexplorer.io/address/' + item.account}>
